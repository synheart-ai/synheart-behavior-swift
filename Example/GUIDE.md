...
behavior.setEventHandler { event in
    guard event.eventType == .tap else { return }
    let duration  = event.metrics["tap_duration_ms"] as? Int   // 120
    let longPress = event.metrics["long_press"]    as? Bool    // false
...
```swift
behavior.setEventHandler { event in
    guard event.eventType == .scroll else { return }
    let velocity     = event.metrics["velocity"]            as? Double // 150.5
    let acceleration = event.metrics["acceleration"]        as? Double // 25.3
    let direction    = event.metrics["direction"]           as? String // "down"
    let reversal     = event.metrics["direction_reversal"]  as? Bool   // false
...
```swift
behavior.setEventHandler { event in
    guard event.eventType == .typing else { return }
    let cadence  = event.metrics["typing_cadence"]            as? Double
    let gapRatio = event.metrics["gap_ratio"]                 as? Double
    let backsp   = event.metrics["backspace_count"]           as? Int
...
```swift
behavior.setEventHandler { event in
    guard event.eventType == .appSwitch else { return }
    let action = event.metrics["action"] as? String  // "foreground" / "background"
...
do {
    ...
    behavior.setEventHandler { event in
        print("Event: \(event.eventType), payload: \(event.metrics)")
    }
