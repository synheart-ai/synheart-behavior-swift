...
behavior.setEventHandler { event in
    ...
    switch event.type {
    case .scroll:
        let velocity = event.payload["velocity"] as? Double
        print("Scroll velocity: \(velocity ?? 0) px/s")
    case .tap:
        let duration = event.payload["tap_duration_ms"] as? Int
        let longPress = event.payload["long_press"] as? Bool
        print("Tap duration: \(duration ?? 0) ms, long press: \(longPress ?? false)")
    case .swipe:
        let direction = event.payload["direction"] as? String
        let velocity = event.payload["velocity"] as? Double
