...
final class SynheartBehaviorTests: XCTestCase {
    ...
    func testThrowsErrorWhenNotInitialized() {
        XCTAssertThrowsError(try sdk.startSession().sessionId) { error in
    ...
    func testStartSession() throws {
        try sdk.initialize()
        let sessionId = try sdk.startSession().sessionId
    ...
    func testStartSessionWithCustomId() throws {
        ...
        let sessionId = try sdk.startSession(sessionId: customId).sessionId
    ...
    func testEndSessionWithInvalidId() throws {
        try sdk.initialize()
        _ = try sdk.startSession().sessionId
    ...
    func testEventHandlerReceivesEvents() throws {
        ...
        XCTAssertEqual(receivedEvent?.eventType, .typing)
    }
    ...
    func testBehaviorEventCreation() {
        ...
        XCTAssertEqual(event.eventType, .typing)
        XCTAssertFalse(event.eventId.isEmpty)
        XCTAssertEqual(event.metrics["typing_speed"] as? Double, 5.5)
        XCTAssertEqual(event.metrics["typing_tap_count"] as? Int, 10)
    }
    ...
    func testBehaviorEventFactoryScroll() {
        ...
        XCTAssertEqual(event.eventType, .scroll)
        XCTAssertEqual(event.metrics["velocity"] as? Double, 120.5)
        XCTAssertEqual(event.metrics["acceleration"] as? Double, 10.0)
        XCTAssertEqual(event.metrics["direction"] as? String, "up")
        XCTAssertEqual(event.metrics["direction_reversal"] as? Bool, true)
    }
    ...
    func testBehaviorEventFactoryTap() {
        ...
        XCTAssertEqual(event.eventType, .tap)
        XCTAssertEqual(event.metrics["tap_duration_ms"] as? Double, 85)
        XCTAssertEqual(event.metrics["long_press"] as? Bool, false)
    }
    ...
    func testBehaviorEventFactorySwipe() {
        ...
        XCTAssertEqual(event.metrics["direction"] as? String, "left")
        XCTAssertEqual(event.metrics["distance_px"] as? Double, 200.0)
        XCTAssertEqual(event.metrics["velocity"] as? Double, 500.0)
    }

    func testBehaviorEventFactoryNotification() {
        let event = BehaviorEvent.notification(sessionId: "s1", action: "ignored")

        XCTAssertEqual(event.eventType, .notification)
        XCTAssertEqual(event.metrics["action"] as? String, "ignored")
    }

    func testBehaviorEventFactoryCall() {
        let event = BehaviorEvent.call(sessionId: "s1", action: "answered")

        XCTAssertEqual(event.eventType, .call)
        XCTAssertEqual(event.metrics["action"] as? String, "answered")
    }
    ...
    func testBehaviorEventFactoryTyping() {
        ...
        XCTAssertEqual(event.eventType, .typing)
        XCTAssertEqual(event.metrics["typing_tap_count"] as? Int, 45)
        XCTAssertEqual(event.metrics["typing_speed"] as? Double, 3.2)
        XCTAssertEqual(event.metrics["typing_burstiness"] as? Double, 0.6)
        XCTAssertEqual(event.metrics["deep_typing"] as? Bool, true)
    }

    func testBehaviorEventFactoryClipboard() {
        let event = BehaviorEvent.clipboard(sessionId: "s1", action: "copy", context: "textField")

        XCTAssertEqual(event.eventType, .clipboard)
        XCTAssertEqual(event.metrics["action"] as? String, "copy")
        XCTAssertEqual(event.metrics["context"] as? String, "textField")
    }

    func testBehaviorEventFactoryAppSwitch() {
        let event = BehaviorEvent.appSwitch(sessionId: "s1")

        XCTAssertEqual(event.eventType, .appSwitch)
    ...
    func testBehaviorEventFactoryAppSwitchWithAction() {
        ...
        XCTAssertEqual(event.eventType, .appSwitch)
        XCTAssertEqual(event.metrics["action"] as? String, "session_stability")
        XCTAssertEqual(event.metrics["stability_index"] as? Double, 0.9)
    }
